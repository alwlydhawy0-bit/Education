 ...
 *
 * It carries the `inBottomNav` destinations from `navigation.js` rather than
 * its own list, so a label reworded in one place cannot leave the sidebar and
 * the bottom bar calling the same page different things — a mismatch nobody
 * sees on a desktop review, because only one of the two is ever on screen.
 *
  ...
  return (
    ...
    >
      <ul className="card-surface flex items-stretch justify-around gap-1 p-1.5">
        {BOTTOM_NAV_ITEMS.map(({ id, label, shortLabel, path, Icon }) => (
          <li key={id} className="min-w-0 flex-1">
            {/*
              `NavLink` sets `aria-current="page"` itself, and `end` on the root
              path only — `/` is a prefix of every route, so without it the home
              cell would stay lit everywhere; with it on every cell, "الدورات"
              would go dark the moment a course page opened.
            */}
            <NavLink
              to={path}
              end={path === '/'}
            ...
            >
              ...
              {/*
                The visible text may be the short form, but the ACCESSIBLE name
                is always the full one: a screen reader should hear "الملف
                الشخصي", not an abbreviation invented for a 70px cell.
