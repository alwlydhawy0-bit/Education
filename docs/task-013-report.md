...
5. **Revocation rotates.** Unpublishing sets the flag false _and_ mints a new
...
its _shape_ is recorded, because a burst of slug-shaped misses is somebody
...
meet a policy that argues with them. Publishing _is_ refused when nothing public
...
no behavioural test _could_ fail. What the source-text assertion catches is the
...
unit layer. _A test that establishes a precondition is not a test of the
control._ The row is now written directly in the HTTP suite and fetched through
...
1. **A teacher rewrote a child's `description_markdown`.** The
   ...
   invited. Fixed with `student_project_review_guard`. _Prose describing a
   control is not a control_ — that is the lesson, and the comment now says so.
...
5. **VULN-055 — every public page returned 404.** The resolver joined `users`
   ...
   asked for one. _When a control fails because a field is hard to obtain
   safely, ask whether the field belongs there at all._
6. **VULN-056 — the only anonymous route stood on a single gate.** The public
   ...
   first, to anybody, for any key. _An argument for a single gate is strongest
   exactly where a single gate is least affordable._
